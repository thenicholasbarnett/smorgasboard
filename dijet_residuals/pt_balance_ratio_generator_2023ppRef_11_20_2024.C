...
void save_finals(TH1D *h, TString ptlo, TString pthi, TString hname){
    ...
    // tline
    int binxmax = h_c->FindLastBinAbove(h_c->GetBinLowEdge(1));
    TLine *line1 = new TLine(h_c->GetBinLowEdge(1),1,5.2,1);
    // TLine *line1 = new TLine(h_c->GetBinLowEdge(1),1,(h_c->GetBinLowEdge(binxmax)+h_c->GetBinWidth(binxmax)),1);
    line1->SetLineWidth(1);
    line1->SetLineColor(kBlack);
    line1->SetLineStyle(2);
...
void pt_balance_ratio_generator_2023ppRef_11_20_2024()
{
    ...
    // ptslices
    // number of pt slices
    const Int_t ptslicenum = 6;
    const Int_t ptslicenum1 = 7;
    // the low and high pt values for each pt slice
    double pts[ptslicenum1] = {15,25,55,80,120,170,1000};
    double ptlow[ptslicenum] = {15,25,55,80,120,170};
    double pthigh[ptslicenum] = {25,55,80,120,170,1000};
    
    // eta slices
    // number of eta slices
    const Int_t etaslicenum = 18;
    const Int_t etaslicenum1 = 19;
    
    // new binnning
    double etas[etaslicenum1] = {0,0.261,0.522,0.783,1.044,1.305,1.479,1.653,1.93,2.172,2.322,2.5,2.65,2.853,2.964,3.139,3.489,3.839,5.191};
    double etahigh[etaslicenum] = {0.261,0.522,0.783,1.044,1.305,1.479,1.653,1.93,2.172,2.322,2.5,2.65,2.853,2.964,3.139,3.489,3.839,5.191};
    double etalow[etaslicenum] = {0,0.261,0.522,0.783,1.044,1.305,1.479,1.653,1.93,2.172,2.322,2.5,2.65,2.853,2.964,3.139,3.489,3.839};
    ...
    // old eta slices
    // number of eta slices
    const Int_t etaslicenum_ = 36;
    const Int_t etaslicenum1_ = 37;
    double etas_[etaslicenum1_] = {-5.191,-3.839,-3.489,-3.139,-2.964,-2.853,-2.65,-2.5,-2.322,-2.172,-1.93,-1.653,-1.479,-1.305,-1.044,-0.783,-0.522,-0.261,0,0.261,0.522,0.783,1.044,1.305,1.479,1.653,1.93,2.172,2.322,2.5,2.65,2.853,2.964,3.139,3.489,3.839,5.191};
    double etahigh_[etaslicenum_] = {-3.839,-3.489,-3.139,-2.964,-2.853,-2.65,-2.5,-2.322,-2.172,-1.93,-1.653,-1.479,-1.305,-1.044,-0.783,-0.522,-0.261,0,0.261,0.522,0.783,1.044,1.305,1.479,1.653,1.93,2.172,2.322,2.5,2.65,2.853,2.964,3.139,3.489,3.839,5.191};
    double etalow_[etaslicenum_] = {-5.191,-3.839,-3.489,-3.139,-2.964,-2.853,-2.65,-2.5,-2.322,-2.172,-1.93,-1.653,-1.479,-1.305,-1.044,-0.783,-0.522,-0.261,0,0.261,0.522,0.783,1.044,1.305,1.479,1.653,1.93,2.172,2.322,2.5,2.65,2.853,2.964,3.139,3.489,3.839};
    ...
    TH1D *hptslicesR_final_symm[ptslicenum];

    // pointing fi to the file holding the jet info of interest

    TFile *fidata = TFile::Open("avals_HP_2023ppRef_11_11_2024_0.root","read");
    // TFile *fidata = TFile::Open("avals_ZB_2023ppRef_11_18_2024_0.root","read");
    TFile *fimc = TFile::Open("avals_MC_2023ppRef_11_11_2024_0.root","read");

    // making an output root file to store stuff
    // TFile *f1 = new TFile("pt_balance_ratios_HP_11_18_2024.root", "recreate");
    TFile *f1 = new TFile("pt_balance_ratios_ZB_11_18_2024_SF.root", "recreate");
    f1->cd();
    ...
    for(unsigned int a=0; a<alphabinnum; a++){
        for(unsigned int p=0; p<ptslicenum; p++){
            ...
            if(a==0){
                ...
                hptslicesR_final_symm[p] = new TH1D(bhtitle00+"_symm","",etaslicenum_,etas_);
            }
    ...
    for(unsigned int p=0; p<ptslicenum; p++){
        for(unsigned int q=0; q<etaslicenum; q++){
            ...
            halpha1[p][q]->Fit(fit1[p][q], "QR");
            hptslicesR_final[p]->SetBinContent(q1, fit1[p][q]->GetParameter(0));
            hptslicesR_final[p]->SetBinError(q1, fit1[p][q]->GetParError(0));

            hptslicesR_final_symm[p]->SetBinContent(q1_, fit1[p][q]->GetParameter(0));
            hptslicesR_final_symm[p]->SetBinError(q1_, fit1[p][q]->GetParError(0));
            hptslicesR_final_symm[p]->SetBinContent(q2, fit1[p][q]->GetParameter(0));
            hptslicesR_final_symm[p]->SetBinError(q2, fit1[p][q]->GetParError(0));
        }
    }
    ...
    for(unsigned int p=0; p<ptslicenum; p++){
        ...
        save_finals(hptslicesR_final_symm[p], ptlo, pthi, dhtitle20+"_symm");
