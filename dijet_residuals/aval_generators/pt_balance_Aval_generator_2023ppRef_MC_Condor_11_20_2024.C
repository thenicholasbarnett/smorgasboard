...
void pt_balance_Aval_generator_2023ppRef_MC_Condor_11_20_2024(TString input, TString output)
{   
...
// INITIALIZING HISTOGRAMS //
    ...
    // event parameters and total jet momenta
...
// DETERMINING CORRECTED LEADING & SUBLEADING JET INDICES //
            ...
            // finding leading jet index
            // looping through the jets in the event
            for(unsigned int j=0; j<nref; j++){
                // only when the jet pt is highest will it replace the current iterator
                // in the case this is never true the original leading jet would still be the leading jet and index would be 0
...
// TAGGING THE LEADING OR SUBLEADING JET //
...
// DETERMING A VALUES FOR EVENTS WITH MORE THAN 2 JETS //

            // pt balance study for more than two jet events
            if(nref>2){

                // determining index of third highest pt jet in event
                // initializing third leading jet index
                int thirditer = 2;
                ...
                // generating A value iff the requirements are met
                // requirements are phi separation, subleading jet pt cut, and tag jet must be in the barrel
                if((TMath::ACos(TMath::Cos(jtphi[leaditer]-jtphi[subleaditer]))>2.7)&&(jtcorrpt[subleaditer]>ptcut)&&(TMath::Abs(jteta[tagiter])<1.3)){
                        ...
                        if(((alphas[a]/2)*(jtcorrpt[probeiter]+jtcorrpt[tagiter]))>jtcorrpt[thirditer]){
                            ...
                            for(unsigned int p=0; p<ptslicenum; p++){
                                ...
                                if((ptavg>ptlow[p])&&(ptavg<pthigh[p])){
                                    ...
                                    ptslicesA[a][p]->Fill(jteta[probeiter],Aval, w);
                                }
                            }
                            ...
                            for(unsigned int q=0; q<etaslicenum; q++){
                                ...
                                if((jteta[probeiter]>etalow[q])&&(jteta[probeiter]<etahigh[q])){
                                    ...
                                    etaslicesA[a][q]->Fill(ptavg,Aval, w);
                                }
                            }
                        }
                    }
            ...
            if(((TMath::Abs(jteta[leaditer])<1.3)||(TMath::Abs(jteta[subleaditer])<1.3))&&(jtcorrpt[subleaditer]>ptcut)&&(nref<3)&&(TMath::ACos(TMath::Cos(jtphi[leaditer]-jtphi[subleaditer]))>2.7)){
